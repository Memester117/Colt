...
//#define WAVING_DEADBUSH          //looks odd when enabled, will be fixed soon

#define ENTITY_DEADBUSH      32.0
...
void main() {
	...
	*/
	...
	if ( mc_Entity.x == ENTITY_LEAVES )
			position.xyz += calcMove(worldpos.xyz, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(1.0,0.2,1.0), vec3(0.0,0.0,0.0));
	#endif
	#ifdef WAVING_DEADBUSH
	if ( mc_Entity.x == ENTITY_DEADBUSH )
			position.xyz += calcMove(worldpos.xyz, 0.0041, 0.070, 0.0044, 0.0038, 0.0063, 0.0000, vec3(1.25,0.55,1.25), vec3(0.0,0.0,0.0));
	#endif
	if (istopv > 0.9) {
	#ifdef WAVING_GRASS
	if ( mc_Entity.x == ENTITY_TALLGRASS)
			position.xyz += calcMove(worldpos.xyz, 0.0041, 0.0070, 0.0044, 0.0038, 0.0063, 0.0000, vec3(2.5,1.1,2.5), vec3(0.4,0.4,0.4));
	#endif
