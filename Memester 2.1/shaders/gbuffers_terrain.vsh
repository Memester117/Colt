...
void main() {
	...
	// Don't use POM on glass blocks.
	if (mc_Entity.x == 20.0 || mc_Entity.x == 102.0) nGlassBlock = 0.0;
	
	if (mc_Entity.x == 10.0 || mc_Entity.x == 11.0 || mc_Entity.x == 89.0) lightSources = 1.0;
	...
	viewVector = normalize(tbnMatrix * viewVector) * nGlassBlock;
