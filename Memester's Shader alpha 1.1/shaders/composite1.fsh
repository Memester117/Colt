...
Do not mess with the code until you have read the LICENSE.txt contained in the root directory of this shaderpack!
...
void 	CalculateClouds2 (inout vec3 color, inout SurfaceStruct surface)
{
	...
	//{
		...
		const int numSteps = 62;
		const float numStepsF = 62.0f;
...
vec4 CloudColor(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector, in float altitude, in float thickness)
{
	...
	float cloudUpperHeight = cloudHeight + (cloudDepth / 3.0f);
	float cloudLowerHeight = cloudHeight - (cloudDepth / 3.0f);
...
void CloudPlane(inout SurfaceStruct surface)
{
	...
	float cloudsThickness = 530.0f;

	float cloudsUpperLimit = cloudsAltitude + cloudsThickness * 1.0f;
	float cloudsLowerLimit = cloudsAltitude - cloudsThickness * 1.0f;
...
void main() {
	...
	//Apply lightmaps to albedo and generate final shaded surface
	vec3 finalComposite = final.sunlight 			* 1.9f 	* 1.5f * sunlightMult				//Add direct sunlight
