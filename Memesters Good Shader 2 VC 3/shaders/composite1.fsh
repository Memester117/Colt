...
//#define NO_LEAVE_GRASS_LIGHTING			//Enable this to "fix" brightly lit tree leaves in the far distance, Best fix is to increase shadow distance past 200, line 153 
...
#define CLOUD_COVERAGE 0.59f + rainy * 0.335f;			//to increase the 2Dclouds:" 0.59f + rainy * 0.35f " is Default when not using 3DClouds," 0.5f + rainy * 0.35f " is best for when using 2D and 3D clouds
...
float   CalculateDirectLighting(in SurfaceStruct surface) {
    ...
    if (surface.NdotL > -0.01f) {
            return surface.NdotL * 0.5f + 0.5f;
        } else {
            return abs(surface.NdotL) * 1.0f;
        } 
...
#ifdef MORNING_FOG
		  fogDensity += 0.04f * timeSunrise * 0.25;
#endif

#ifdef 	EVENING_FOG
		  fogDensity += 0.04f * timeSunset * 0.25;
#endif
...
vec4 CloudColor(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector)
{
	...
	else
	{
		...
		vec3 colorDirect = colorSunlight * 20.0f;
...
void 	CalculateClouds (inout vec3 color, inout SurfaceStruct surface)
{
		...
		float cloudHeight = 350.0f;
		float cloudDepth  = 260.0f;
