...
//----------Shadows----------//
	const int shadowMapResolution = 110;		//shadowmap resolution
	const float shadowDistance = 20.0;		//draw distance of shadows
	#define SHADOW_DARKNESS 0.2		//shadow darkness levels, lower values mean darker shadows, see .vsh for colors /0.25 is default
...
//----------Lighting----------//
	...
	#define TORCH_COLOR_LIGHTING 2.4,0.50,0.1 	//Torch Color RGB - Red, Green, Blue / vec3(2.4,0.50,0.1) is default
		#define TORCH_ATTEN 2.0					//how much the torch light will be attenuated (decrease if you want the torches to cover a bigger area))/3.0 is default
		#define TORCH_INTENSITY 0.06				//torch light intensity /0.06 is default
		#define TORCH_MULT 2.2					//Torch lightmap attenuation function, where x is torch light from minecraft lightmap  : ((x*mult)^atten)*intensity
...
//#define GODRAYS
	...
	const float	sunPathRotation	= -0.0f;		//determines sun/moon inclination /-40.0 is default - 0.0 is normal rotation
//----------End of Visual----------//
