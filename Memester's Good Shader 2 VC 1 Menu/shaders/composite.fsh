...
vec4 CloudColor(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector)
{
	...
	float cloudHeight = 460.0f;
	float cloudDepth  = 460.0f;
	...
	else
	{
		...
		float noise  = 	Get3DNoise(p * vec3(1.0f, 0.5f, 1.0f) + vec3(0.0f, t * 0.01f, 0.0f));	p *= 3.0f;	p.x -= t * 0.057f;	vec3 p2 = p;
			  noise += (1.0f - abs(Get3DNoise(p) * 1.0f - 0.5f)) * 0.9f;				p *= 3.0f;	p.xz -= t * 0.035f;	vec3 p3 = p;
			  noise += (1.0f - abs(Get3DNoise(p) * 3.0f - 1.0f)) * 0.12f;				p *= 3.0f;	p.xz -= t * 0.035f;
		...
		vec3 colorDirect = colorSunlight * 0.56f;
...
{
	//if (texcoord.s < 0.5f && texcoord.t < 2.5f)
	//{
		surface.cloudAlpha = 4.0f;
		...
		float cloudHeight = 700.0f;
		float cloudDepth  = 400.0f;
		...
		float rayDepth = startingRayDepth;
			  ...
			  //rayDepth += texture2D(noisetex, texcoord.st * (viewWidth / noiseTextureResolution, viewHeight / noiseTextureResolution)).x * 2.1f;
			  //rayDepth += CalculateDitherPattern2() * 1.1f;
		...
		vec4 cloudSum = vec4(0.0f);
			 cloudSum.rgb = colorSkylight * 0.00000002f;
...
vec4 CloudColor(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector, in float altitude, in float thickness)
{
	...
	float t = frameTimeCounter * 100.0f;
		  //t *= 0.001;
	p.x -= t * 0.01f;
...
void CloudPlane(inout SurfaceStruct surface)
{
	...
	float cloudsThickness = 250.0f;

	float cloudsUpperLimit = cloudsAltitude + cloudsThickness * 2.5f;
	float cloudsLowerLimit = cloudsAltitude - cloudsThickness * 2.5f;
	...
	if (cameraPosition.y < cloudsLowerLimit)
	{
		...
		float stepSize = 35.5f;
		planeHeight -= cloudsThickness * 3.85f;
