...
const float 	shadowIntervalSize 		 = 12.f;
const int 		shadowMapResolution 	 = 8192;		    // Shadowmap resolution.
const float 	shadowDistance 			 = 144.0f;		// Draw distance of shadows.
...
float iswet = wetness*pow(sky_lightmap,10.0)*sqrt(0.5+max(dot(normal,normalize(upPosition)),0.0));
	...
	float waterH(vec3 posxz) {
		...
		for (int i = 1; i < 6; i++) {
			wave -= d*factor*sin( (1/factor)*px*py*size + 0.0*frameTimeCounter*speed);
			factor /= 2;
		}
		...
		for (int i = 1; i < 6; i++) {
			wave2 -= d*factor*cos( (1/factor)*px*py*size + 0.0*frameTimeCounter*speed);
			factor /= 2;
		}
